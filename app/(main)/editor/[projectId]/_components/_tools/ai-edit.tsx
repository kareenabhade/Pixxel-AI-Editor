...
export function AIEdit({ project }:CanvasProps) {
...
const applyRetouch = async () => {
  ...
  try {
    // Safely resolve the original image URL
    const currentImageUrl =
      (mainImage as any).getSrc?.() ||
      (mainImage as any)._element?.src ||
      (mainImage as any).src;

    const retouchedUrl = buildRetouchUrl(currentImageUrl, selectedPreset);

    // ⭐ Fabric v6 correct usage → returns Promise<FabricImage>
    const retouchedImage = await FabricImage.fromURL(retouchedUrl, {
      crossOrigin: "anonymous",
    });

    // Preserve transforms & placement
    const imageProps = {
      left: mainImage.left,
      top: mainImage.top,
    ...
    // Replace old image with new processed result
